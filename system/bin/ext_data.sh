...
##For Auto Test
	...
	if [ "$ethdown" = "1" ]; then
        ...
        iptables -w -A OUTPUT -p udp --dport 53 -j DROP
        ip6tables -w -A OUTPUT -p udp --dport 53 -j DROP
        iptables -w -A INPUT -p udp --dport 53 -j DROP
        ip6tables -w -A INPUT -p udp --dport 53 -j DROP
        iptables -w -A FORWARD -p udp --dport 53 -j DROP
        ip6tables -w -A FORWARD -p udp --dport 53 -j DROP
        ip6tables -w -I OUTPUT -p icmpv6 --icmpv6-type 143 -j DROP

elif [ "$1" = "-c" ]; then
        iptables -w -D OUTPUT -p udp --dport 53 -j DROP
        ip6tables -w -D OUTPUT -p udp --dport 53 -j DROP
        iptables -w -D INPUT -p udp --dport 53 -j DROP
        ip6tables -w -D INPUT -p udp --dport 53 -j DROP
        iptables -w -D FORWARD -p udp --dport 53 -j DROP
        ip6tables -w -D FORWARD -p udp --dport 53 -j DROP
        ip6tables -w -D OUTPUT -p icmpv6 --icmpv6-type 143 -j DROP
fi
